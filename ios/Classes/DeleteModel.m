...
+ (void)handleEvent:(NSString *)text result:(FlutterResult)result {
    ...
    Boolean isModelPresent = [localModels containsObject:[FIRTranslateRemoteModel translateRemoteModelWithLanguage:modelName]];
    
    if (isModelPresent) {
        [[FIRModelManager modelManager] deleteDownloadedTranslateModel:[FIRTranslateRemoteModel translateRemoteModelWithLanguage:modelName]
